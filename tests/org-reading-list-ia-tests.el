...
(ert-deftest org-reading-list-ia-test-probe-open ()
  (let (captured-open)
    (cl-letf (((symbol-function 'org-reading-list-ia--search)
               (lambda (_a _t _rows &optional open)
                 (setq captured-open open)
      ...
      (let ((hit (org-reading-list-ia--probe-open "Soulé, Frank" "The Annals")))
        ;; The probe asked for open-only results.
        (should (eq captured-open t))
  ...
  (let ((entries (list
    ...
    (cl-letf (((symbol-function 'org-reading-list-ia--probe-open)
      ...
      (let ((res (org-reading-list-ia--report-rows entries)))
        ;; have1900 has a local copy -> counted, never probed.
        (should (= (plist-get res :have-copy) 1))
        ;; soule + none were probed.
        (should (= (plist-get res :checked) 2))
        (should (null (plist-get res :errors)))
        ;; Only soule produced an open hit.
        (should (= (length (plist-get res :rows)) 1))
        (should (equal (plist-get (car (plist-get res :rows)) :citekey)
                       "soule1966"))
        (should (equal (plist-get (plist-get (car (plist-get res :rows))
                                             :earliest)
                                  :identifier)
                       "i1855"))))))
...
(ert-deftest org-reading-list-ia-test-scan-headings-superseded ()
  (with-temp-buffer
    (org-mode)
    (insert "* Books\n"
            "** A\n:PROPERTIES:\n:CUSTOM_ID: a1\n:TITLE: A\n:AUTHOR: X, Y\n"
